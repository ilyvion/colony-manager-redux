...
public static class DateExtensions
{
    ...
    public static string ToStringTicksToPeriodVerboseFull(this int numTicks, bool allowHours = true)
    {
        ...
        if (allowHours)
        {
            ...
            else if (hoursFloat > 0f)
            {
                if (hoursFloat > 1f)
                {
                    var roundedHours = Mathf.RoundToInt(hoursFloat);
                    parts.Add(
                        roundedHours == 1
                            ? "Period1Hour".Translate()
                            : "PeriodHours".Translate(roundedHours)
                    );
                }
                else if (Math.Round(hoursFloat, 1) == 1.0)
                {
                    parts.Add("Period1Hour".Translate());
                }
                else
                {
                    parts.Add(
                        "PeriodHours".Translate(
                            hoursFloat.ToString("0.#", CultureInfo.InvariantCulture)
                        )
                    );
                }
            }
