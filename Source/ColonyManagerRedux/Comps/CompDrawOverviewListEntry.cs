...
public abstract class DrawOverviewListEntryWorker<T> : DrawOverviewListEntryWorker where T : ManagerJob
{
    ...
    public sealed override void ChangeDrawListEntryParameters(
        ManagerJob job,
        ref DrawOverviewListEntryParameters parameters) => ChangeDrawListEntryParameters((T)job, ref parameters);

    /// <inheritdoc/>
    public sealed override void DrawOverviewListEntry(
        ManagerJob job,
        ref Vector2 position,
        float width) => DrawOverviewListEntry((T)job, ref position, width);
