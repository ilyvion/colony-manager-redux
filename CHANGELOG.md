...
- In the Production tab's recipe list, a recipe usable at many workbenches could list more workbenches than its row had room for, spilling its text into the row below instead of growing the row to fit.
- Saving the game after deleting manager jobs could log a warning about a manager job being referenced but not saved, caused by log entries still pointing at the deleted job.
...
- Hovering over a mineral in the Mining tab's mineral list would crash every frame if the mineral's chunk had no 'butcher products', as happens with some modded minerals whose chunks instead have smelt products (e.g. Alpha Biomes' Ancient Metal returning steel chunks).
- Toggling the 'lock to map' padlock on a Foraging or Hunting job's plant/animal list could let you allow entries in the trigger threshold that had no matching item in the threshold's own allow list, since the threshold's parent filter wasn't kept in sync with the lock state. The same padlocks on Foraging, Hunting, Forestry, and Mining jobs also now immediately refresh their plant/animal/mineral/building lists and threshold allow list when toggled, instead of only doing so on the next manual refresh.
...
- A job's history graph now has a 'Clear history for this job' option in its right-click menu, letting you wipe its recorded history data without affecting the job itself. Implements [#20](https://github.com/ilyvion/colony-manager-redux/issues/20).
- Mining, Hunting, Forestry (logging), Foraging, and Livestock's taming job now have an 'Invert area' checkbox next to their area of operation selector. Enable it to flip the restriction around, so the job only operates _outside_ the selected area instead of inside it — e.g. mine everywhere except the Home area, rather than having to draw and maintain a separate area covering everywhere but Home. Implements [#25](https://github.com/ilyvion/colony-manager-redux/issues/25).
- Mining, Hunting, Forestry (logging), and Foraging's progress bars (both in the job list and on the Overview tab) now show a dimmer overlay past the current stock level, representing what's already designated but not yet collected — mined chunks, downed corpses, or trees marked for felling. This makes it easier to tell at a glance whether a job is already on track to meet its target without opening its settings.
- Livestock jobs now have an 'avoid culling named animals' toggle next to the existing Trained/Pregnant/Bonded culling options, letting the manager skip animals that have been given a real name (such as a previously bonded animal) rather than an auto-generated one, so they don't get accidentally marked for slaughter when culling excess livestock. Off by default; there's also a matching default in the mod settings' per-animal-type defaults. Implements [#29](https://github.com/ilyvion/colony-manager-redux/issues/29).
- Added a Production manager job: pick a recipe and the manager will keep an eligible work table running it for you instead of you having to babysit individual bills. It has two modes: 'Maintain stock' keeps a target amount of the recipe's own product in stock, splitting whatever's still needed exactly across every eligible work table so several benches working at once don't overshoot your target; 'Consume surplus' instead runs the recipe whenever some other resource of your choosing crosses a threshold, independent of what the recipe actually produces — e.g. only turn cotton into dusters once you're sitting on a surplus of cotton. You can restrict which work tables the job uses (all of them, an area, or a hand-picked list), and set a skill range, ingredient search radius, and storage destination for its bills the same way you would configure an ordinary bill, plus separately restrict which raw materials it's allowed to consume. Recipes without one single fixed product — butchery, stonecutting, smelting — are fully supported, and the recipe picker only ever offers recipes you can currently actually build (i.e. already unlocked via research/ideology/faction, with a work table for it built). You can also run more than one job against the same recipe, e.g. maintaining a stock of steel knives while a separate job turns surplus cotton into dusters on the same tailoring bench. Bills a Production job places are shown as read-only in the vanilla bill configuration window (with a small icon marking them as manager-controlled) so you don't accidentally change settings there that the manager will just overwrite again; the bill reordering buttons stay usable, since bill order doesn't conflict with anything the manager manages. Implements [#5](https://github.com/ilyvion/colony-manager-redux/issues/5).
- A Production job can offer to swap to a different recipe that produces the same thing, via an 'Other recipes available' button in its recipe panel — handy when you unlock a better recipe for the same item later and want to switch to it without losing the job's target count or history.
- Production jobs can be linked together so raw materials flow automatically down a production chain: click the chain-link icon next to an ingredient on a job's ingredient list to link it to an existing production job that makes it, or create a new one on the spot. A linked producer job can then be set to automatically size its own target to match what its linked consumer(s) actually need — choosing whether to add every consumer's demand together or just match the hungriest one — and to automatically restrict its own ingredients to only what's actually wanted downstream. For example, linking a 'cook simple meals' job's meat ingredient to a 'butcher creature' job keeps the butcher job's target in sync with how much meat your meal production actually needs, without you having to retune it by hand.
...
- Applying a job template or job save (or building your first manager station on a map with an auto-apply default template configured) that was created under a different set of mods no longer pops up RimWorld's save-game 'Mods mismatch' screen, which was confusing since it looks and reads like you're loading an actual save. Manually applying a template/save through the Import/Export tab now shows a Colony Manager Redux-specific warning instead, still listing the added/missing/shared mods so you know what might be affected. A default template that would trigger this warning is no longer applied automatically when you place your first manager station — you'll instead get a message telling you it was skipped, with a pointer to apply it manually if you still want it.
- If you use AAAA with the danger/evacuation area feature turned on, an area switching back to normal from danger mode could, in rare cases, throw an error and silently stop restoring areas for the rest of that update instead of just for the affected job.
...
- Manager tabs with long scrollable lists were drawing every single row every frame, even the ones scrolled out of view. On a list with a few dozen entries, this could make the whole window noticeably laggy. Rows outside the visible area are no longer drawn until you scroll them into view.
- Jobs with a very large allowed-targets list (e.g. a hunting or forestry job allowing most or all animals/trees) could make the manager window noticeably laggy just from having it open, since the full comma-separated target list shown under the job's name was being rebuilt and re-measured for ellipsis-truncation from scratch every single frame. That text is now cached and only rebuilt when the allowed targets actually change.
